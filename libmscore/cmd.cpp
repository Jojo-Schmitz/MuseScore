...
void Score::cmdAddInterval(int val, const std::vector<Note*>& nl)
      ...
      for (Note* currNote : _nl) {
            for (Note* n = currNote; n /*&& !contains(tmpnl, n)*/; n = n->tieFor() ? n->tieFor()->endNote() : nullptr) {
      ...
      for (Note* on : tmpnl) {
            ...
            if (on->staff() && on->staff()->isDrumStaff(tick)) {
                  ...
                  nval.pitch = ds->line(on->line() - deltaLine);
                  nval.headGroup = ds->noteHead(nval.pitch);
                  noteValid = ds->isValid(nval.pitch) && nval.headGroup != NoteHead::Group::HEAD_INVALID;
                  }
            else {
                  if (useOctaveRule) {
            ...
            if (!noteValid) {
                  notesToSelect.push_back(toElement(on));
                  continue;
                  }
            ...
            if (selIsList && note)
                  notesToSelect.push_back(toElement(note));
            }
      ...
      if (_is.cr() == toChordRest(_nl.front()->chord()) && selIsSingle)
