...
void BarLine::drawDots(QPainter* painter, qreal x) const
      ...
      if (parent() == 0) {    // for use in palette (always Bravura)
            ...
            qreal offset = (   score()->scoreFont()->name() == "Emmentaler"
                            || score()->scoreFont()->name() == "Gonville"
                            || score()->scoreFont()->name() == "MuseJazz"
                            // above internal, builtin fonts, below external fonts
                            ||score()->scoreFont()->name() == "Ekmelos" // not the other ones from the Ekmelos family though (EkmelosXXedo)
                            ) ? 0.5 * score()->spatium() * mag() : 0;
