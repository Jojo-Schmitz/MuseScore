...
void Stem::layout()
      ...
      if (chord()) {
            ...
            if (st && st->isTabStaff() ) {            // TAB staves
                  ...
                  if ((u && !m) || (!u && m)) { // up-stem
                        // default offsets for all fonts, half a stem-width, fits for most
                        ...
                        if (musicalFont == "AloisenGrooveU") {
                              ...
                              if (symId == SymId::noteheadDoubleWhole) // so far the only font with correct (?) metadata for this notehead
                                    x -= n->headWidth(); // so compensate for the default correction done earlier
                              }
                  ...
                  else { // down-stem, here all fonts seem to agree, half a stem-width;
