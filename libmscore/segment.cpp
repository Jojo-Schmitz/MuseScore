...
//                        updateNoteLines(this, el->track());   TODO::necessary?
            ...
            case ElementType::TIMESIG:
                  Q_ASSERT(_segmentType == SegmentType::TimeSig || _segmentType == SegmentType::TimeSigAnnounce);
                  checkElement(el, track);
                  _elist[track] = el;
      ...
      switch (e->type()) {
            ...
            default: {
                 ...
                 Element* prev = seg->prevElementOfSegment(seg, el, activeStaff);
                  if (prev) {
                        if (prev->isChord() || prev->isRest()
                               || prev->isRepeatMeasure()) {
                   ...
                   if (prev->isChord() || prev->isRest()
                            || prev->isRepeatMeasure() || prev->isNote()) {
...
Element* Segment::lastInPrevSegments(int activeStaff)
      ...
      if (!seg) { //end of staff
            ...
            while (true) {
                  //if (seg->isEndBarLineType() == SegmentType::EndBarLine)
