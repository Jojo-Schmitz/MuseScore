...
#define IS(a,b) \
      ...
      IS(Tie,           TIE)
      IS(Slur,          SLUR)
      IS(Glissando,     GLISSANDO)
      IS(GlissandoSegment,     GLISSANDO_SEGMENT)
      IS(SystemDivider, SYSTEM_DIVIDER)
      IS(RehearsalMark, REHEARSAL_MARK)
      IS(Harmony,       HARMONY)
      IS(Volta,         VOLTA)
      IS(Jump,          JUMP)
      IS(Ottava,        OTTAVA)
      IS(LayoutBreak,   LAYOUT_BREAK)
      IS(Segment,       SEGMENT)
      IS(Tremolo,       TREMOLO)
      IS(System,        SYSTEM)
      IS(Lyrics,        LYRICS)
      ...
      IS(Beam,          BEAM)
      IS(Hook,          HOOK)
      IS(StemSlash,     STEM_SLASH)
      IS(SlurSegment,   SLUR_SEGMENT)
      IS(TieSegment,    TIE_SEGMENT)
      IS(Spacer,        SPACER)
      IS(StaffLines,    STAFF_LINES)
      IS(Ambitus,       AMBITUS)
      IS(Bracket,       BRACKET)
      IS(InstrumentChange, INSTRUMENT_CHANGE)
      IS(StaffTypeChange, STAFFTYPE_CHANGE)
      IS(Hairpin,       HAIRPIN)
      IS(HairpinSegment,HAIRPIN_SEGMENT)
      IS(Bend,          BEND)
      IS(TremoloBar,    TREMOLOBAR)
      IS(RepeatMeasure, REPEAT_MEASURE)
      IS(Tuplet,        TUPLET)
      IS(NoteDot,       NOTEDOT)
      IS(Dynamic,       DYNAMIC)
      IS(InstrumentName, INSTRUMENT_NAME)
      IS(Accidental,    ACCIDENTAL)
      IS(TextLine,      TEXTLINE)
      IS(TextLineSegment,      TEXTLINE_SEGMENT)
      IS(Pedal,         PEDAL)
      IS(PedalSegment,  PEDAL_SEGMENT)
      IS(OttavaSegment, OTTAVA_SEGMENT)
      IS(LedgerLine,    LEDGER_LINE)
      IS(Icon,          ICON)
      IS(VoltaSegment,  VOLTA_SEGMENT)
      IS(Trill,         TRILL)
      IS(TrillSegment,  TRILL_SEGMENT)
      IS(LetRing,       LET_RING)
      IS(LetRingSegment, LET_RING_SEGMENT)
      IS(Vibrato,       VIBRATO)
      IS(PalmMute,      PALM_MUTE)
      IS(PalmMuteSegment, PALM_MUTE_SEGMENT)
      IS(VibratoSegment,  VIBRATO_SEGMENT)
      IS(Symbol,        SYMBOL)
      IS(FSymbol,       FSYMBOL)
      IS(Fingering,     FINGERING)
      IS(NoteHead,      NOTEHEAD)
      IS(LyricsLine,    LYRICSLINE)
      IS(LyricsLineSegment, LYRICSLINE_SEGMENT)
      IS(FiguredBass,   FIGURED_BASS)
      IS(StaffState,    STAFF_STATE)
      IS(Arpeggio,      ARPEGGIO)
      IS(Image,         IMAGE)
      IS(ChordLine,     CHORDLINE)
      IS(FretDiagram,   FRET_DIAGRAM)
      IS(Page,          PAGE)
      IS(Text,          TEXT)
      IS(MeasureNumber, MEASURE_NUMBER)
      IS(MMRestRange,   MMREST_RANGE)
      IS(StaffText,     STAFF_TEXT)
      IS(SystemText,    SYSTEM_TEXT)
      IS(BracketItem,   BRACKET_ITEM)
      IS(Score,         SCORE)
      IS(Staff,         STAFF)
      IS(Part,          PART)
      IS(BagpipeEmbellishment, BAGPIPE_EMBELLISHMENT)
      IS(Lasso,         LASSO)
      IS(Sticking,      STICKING)
#undef IS
